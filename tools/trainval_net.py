...
from lib.model.utils.net_utils import adjust_learning_rate, save_checkpoint, clip_gradient, adjust_learning_rate_warmup
...
if __name__ == '__main__':
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        for step in range(iters_per_epoch):
            if cfg.TRAIN.WARMUP and epoch == 1:
                lr = adjust_learning_rate_warmup(optimizer, args.lr, epoch, step, iters_per_epoch)
            data = next(data_iter)
            ...
            RCNN_loss_cls, RCNN_loss_bbox, RCNN_loss_sign, bbox_sign_prob, \
            ...
            loss = rpn_loss_cls.mean() + rpn_loss_box.mean() \
                   + RCNN_loss_cls.mean() + RCNN_loss_bbox.mean() + RCNN_loss_sign.mean()
            ...
            if step % args.disp_interval == 0:
                ...
                if args.mGPUs:
                    ...
                    # if cfg.TRAIN.EDGE_LOSS:
                    loss_rcnn_edge = RCNN_loss_sign.mean().data[0]
                    ...
                    # if cfg.TRAIN.EDGE_LOSS:
                    loss_rcnn_edge = RCNN_loss_sign.data[0]
                    hard_cnt = torch.sum(rois_label.data.eq(-1))
                    fg_cnt = torch.sum(rois_label.data.ne(0)) - hard_cnt
                    bg_cnt = rois_label.data.numel() - fg_cnt - hard_cnt
                ...
                # if cfg.TRAIN.EDGE_LOSS:
                print("    >>> rcnn_sign %.4f" %(loss_rcnn_edge))
                if args.use_tfboard:
                    info = {
                        'loss': loss_temp,
